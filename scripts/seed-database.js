...
const USER_PERSONAS = [
    {
        type: 'developer',
        bio: () => `Full-stack developer passionate about ${faker_1.faker.helpers.arrayElement(['React', 'Vue', 'Angular', 'Node.js', 'Python', 'Go', 'Rust'])}. ${faker_1.faker.helpers.arrayElement(['Building the future of web3', 'Love clean code and good coffee', 'Open source contributor', 'Always learning new technologies'])}.`,
        interests: ['programming', 'webdev', 'javascript']
    },
    {
        type: 'gamer',
        bio: () => `Avid gamer and ${faker_1.faker.helpers.arrayElement(['streamer', 'content creator', 'esports enthusiast', 'indie game lover'])}. Currently playing ${faker_1.faker.helpers.arrayElement(['Baldur\'s Gate 3', 'Cyberpunk 2077', 'Deep Rock Galactic', 'Hades', 'Hollow Knight'])}.`,
        interests: ['gaming', 'indiegames']
    },
    {
        type: 'artist',
        bio: () => `Digital artist specializing in ${faker_1.faker.helpers.arrayElement(['character design', 'concept art', 'pixel art', 'illustrations', '3D modeling'])}. Commission work available!`,
        interests: ['digitalart', 'gamedev']
    },
    {
        type: 'creator',
        bio: () => `Content creator focused on ${faker_1.faker.helpers.arrayElement(['tech tutorials', 'game reviews', 'art process videos', 'music production'])}. Always collaborating with the community.`,
        interests: ['music', 'digitalart', 'gaming']
    },
    {
        type: 'entrepreneur',
        bio: () => `Building the next big thing in ${faker_1.faker.helpers.arrayElement(['fintech', 'web3', 'gaming', 'social media'])}. Love connecting with other founders and sharing insights.`,
...
function shouldGenerate(probability) {
...
async function generateCommunities() {
    ...
    return allCommunities.map(community => ({
        ...
        icon: faker_1.faker.image.url({ width: 256, height: 256 }),
        banner: faker_1.faker.image.url({ width: 1200, height: 400 }),
...
async function generateServers(users) {
    ...
    return serverTemplates.map(template => ({
        ...
        isPublic: true,
        maxMembers: faker_1.faker.number.int({ min: 1000, max: 100000 }),
        createdAt: faker_1.faker.date.between({
            from: new Date('2023-01-01'),
            to: new Date()
        }),
...
async function generateChannelsForServers(servers) {
...
async function generatePosts(communities, users) {
    console.log(`📝 Generating posts...`);
    const posts = [];
    for (const community of communities) {
        const templates = POST_TEMPLATES[community.name] || POST_TEMPLATES['programming'];
        for (let i = 0; i < SEED_CONFIG.postsPerCommunity; i++) {
            ...
            const post = {
                ...
                score: faker_1.faker.number.int({ min: -5, max: 500 }),
                viewCount: faker_1.faker.number.int({ min: 10, max: 2000 }),
                commentCount: 0, // Will be updated after creating comments
                isPinned: faker_1.faker.datatype.boolean({ probability: 0.05 }),
                nsfw: faker_1.faker.datatype.boolean({ probability: 0.02 }),
                createdAt: faker_1.faker.date.between({
...
async function generateComments(posts, users) {
    console.log(`💬 Generating comments...`);
    const comments = [];
    for (const post of posts) {
        const numComments = faker_1.faker.number.int({ min: 2, max: SEED_CONFIG.commentsPerPost });
        ...
        for (let i = 0; i < numComments; i++) {
            ...
            const comment = {
                ...
                parentId: null,
                content: template,
                score: faker_1.faker.number.int({ min: -2, max: 50 }),
                createdAt: faker_1.faker.date.between({
                    from: post.createdAt,
                    to: new Date()
                }),
            ...
            if (shouldGenerate(0.3)) {
                const numReplies = faker_1.faker.number.int({ min: 1, max: 3 });
                for (let j = 0; j < numReplies; j++) {
                    ...
                    const reply = {
                        ...
                        score: faker_1.faker.number.int({ min: 0, max: 20 }),
                        createdAt: faker_1.faker.date.between({
...
async function generateMessages(channels, users) {
    console.log(`💌 Generating messages...`);
    const messages = [];
    const textChannels = channels.filter(c => c.type === 'GUILD_TEXT');
    for (const channel of textChannels) {
        const channelMembers = randomElements(users, faker_1.faker.number.int({ min: 5, max: 20 }));
        for (let i = 0; i < SEED_CONFIG.messagesPerChannel; i++) {
            const author = randomElement(channelMembers);
            const messageContent = faker_1.faker.helpers.arrayElement([
                ...
                "Check out this awesome article I found: " + faker_1.faker.internet.url(),
                "Quick question - has anyone used " + faker_1.faker.helpers.arrayElement(['React', 'Vue', 'Svelte', 'Angular']) + " for this type of project?",
            ...
            const message = {
                ...
                timestamp: faker_1.faker.date.between({
                    from: channel.createdAt,
                    to: new Date()
                }),
                tts: false,
                mentionEveryone: faker_1.faker.datatype.boolean({ probability: 0.01 }),
                isPinned: faker_1.faker.datatype.boolean({ probability: 0.02 }),
            };
...
async function generateCommunityMemberships(communities, users) {
    console.log(`👥 Generating community memberships...`);
    const memberships = [];
    for (const community of communities) {
        // Each community gets 20-80% of users as members
        const memberCount = faker_1.faker.number.int({
        ...
        for (const user of members) {
            memberships.push({
                ...
                communityId: community.id,
                userId: user.id,
                karma: faker_1.faker.number.int({ min: 0, max: 1000 }),
                joinedAt: faker_1.faker.date.between({
                    from: community.createdAt,
                    to: new Date()
                }),
            });
...
async function generateServerMemberships(servers, users) {
    ...
    for (const server of servers) {
        ...
        for (const user of members) {
            memberships.push({
                ...
                serverId: server.id,
                userId: user.id,
                nickname: faker_1.faker.datatype.boolean({ probability: 0.3 }) ? faker_1.faker.person.firstName() : null,
                joinedAt: faker_1.faker.date.between({
                    from: server.createdAt,
                    to: new Date()
                }),
...
async function generateVotes(posts, comments, users) {
    console.log(`🗳️ Generating votes...`);
    const votes = [];
    // Generate votes for posts
    for (const post of posts) {
        const voterCount = faker_1.faker.number.int({ min: 5, max: 50 });
        ...
        for (const voter of voters) {
            if (voter.id !== post.userId) { // Users can't vote on their own posts
                votes.push({
                    ...
                    postId: post.id,
                    commentId: null,
                    value: faker_1.faker.helpers.weightedArrayElement([
    ...
    for (const comment of comments) {
        if (shouldGenerate(SEED_CONFIG.voteProbability)) {
            const voterCount = faker_1.faker.number.int({ min: 1, max: 10 });
            const voters = randomElements(users, voterCount);
            for (const voter of voters) {
...
async function generateFriendships(users) {
    ...
    for (const user of users) {
        ...
        const friendCount = faker_1.faker.number.int({ min: 0, max: 8 });
        const friends = randomElements(potentialFriends, friendCount);
        for (const friend of friends) {
            // Check if friendship already exists in either direction
            const exists = friendships.some(f => (f.initiatorId === user.id && f.receiverId === friend.id) ||
...
async function generateReactions(messages, users) {
